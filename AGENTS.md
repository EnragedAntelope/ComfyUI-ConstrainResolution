...
- The GitHub Actions pins are commit SHAs; the comment above each one records the version it corresponds to. `Comfy-Org/publish-node-action` is pinned to the commit its `v1` *branch* points at — its `1.0.1` tag is older than that branch and still calls `comfy tracking disable`, which current comfy-cli does not have.
